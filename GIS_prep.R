...
address_wd<-address %>% mutate(School_Code=str_sub(CDSCode,9,14)) %>% filter(Latitude != "No Data")
...
merged<-left_join(vaccine_wd,address_wd,by="School_Code") %>% drop_na() %>% ungroup() %>%
  ...
  mutate(Latitude=as.numeric(Latitude)) %>% mutate(Longitude=as.numeric(Longitude))
